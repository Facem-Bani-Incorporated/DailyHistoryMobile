...
const readCache = async (iso: string): Promise<CacheEntry | null> => {
  try { const raw = await AsyncStorage.getItem(cacheKey(iso)); if (!raw) return null; const e: CacheEntry = JSON.parse(raw); if (Date.now() - e.ts > CACHE_TTL_MS) return null; return e; } catch { return null; }
};
...
const EmptyDay = ({ isToday, theme, onRetry, t }: { isToday: boolean; theme: any; onRetry: () => void; t: (k: string) => string }) => (
  <View style={emptyStyles.wrap}>
    <View style={[emptyStyles.iconRing, { borderColor: theme.gold + '20' }]}>
      <Text style={[emptyStyles.icon, { color: theme.gold }]}>{isToday ? '~' : '\u2014'}</Text>
    </View>
    <Text style={[emptyStyles.title, { color: theme.text }]}>{isToday ? t('today') : t('no_content')}</Text>
    <Text style={[emptyStyles.desc, { color: theme.subtext }]}>{isToday ? t('empty_today_desc') : t('empty_day_desc')}</Text>
    {isToday && (
      <TouchableOpacity onPress={onRetry} activeOpacity={0.7} style={[emptyStyles.retryBtn, { borderColor: theme.gold + '30' }]}>
        <Text style={[emptyStyles.retryTxt, { color: theme.gold }]}>{t('retry').toUpperCase()}</Text>
      </TouchableOpacity>
    )}
  </View>
);
const emptyStyles = StyleSheet.create({
  ...
  desc: { fontSize: 13, textAlign: 'center', lineHeight: 20, opacity: 0.7 },
  retryBtn: { marginTop: 8, paddingVertical: 10, paddingHorizontal: 28, borderRadius: 24, borderWidth: 1 },
  retryTxt: { fontSize: 11, fontWeight: '700', letterSpacing: 2 },
});
...
export default function HomeScreen() {
  ...
  const [allCollectedEvents, setAllCollectedEvents] = useState<any[]>([]);
  ...
  useEffect(() => {
    fetchData(0).then(async ({ data, empty }) => {
      ...
      try {
        ...
        } else { scheduleMidnightNotification('\ud83d\udcc5 Daily History', 'A new historical event awaits you!'); }
  ...
  const navigateToOffset = useCallback(async (newOffset: number, direction: 'left' | 'right') => {
    if (navigating.current || newOffset > MAX_FUTURE_OFFSET) return;
    navigating.current = true;
    haptic('light');
    const exitTo = direction === 'left' ? -W * 0.4 : W * 0.4;
    const enterFrom = direction === 'left' ? W * 0.3 : -W * 0.3;
    await new Promise<void>(resolve => Animated.parallel([
      Animated.timing(cardX, { toValue: exitTo, duration: 200, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.timing(cardOpacity, { toValue: 0, duration: 180, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.timing(dateSlide, { toValue: direction === 'left' ? -30 : 30, duration: 200, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
    ]).start(() => resolve()));
    cardX.setValue(enterFrom); dateSlide.setValue(direction === 'left' ? 20 : -20);
    setDayOffset(newOffset);
    const { data, empty } = await fetchData(newOffset);
    setEvents(data); setIsEmpty(empty);
    await new Promise<void>(resolve => Animated.parallel([
      Animated.spring(cardX, { toValue: 0, tension: 200, friction: 26, useNativeDriver: true }),
      Animated.timing(cardOpacity, { toValue: 1, duration: 220, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.spring(dateSlide, { toValue: 0, tension: 200, friction: 26, useNativeDriver: true }),
    ]).start(() => resolve()));
  ...
  const pan = useRef(PanResponder.create({
    ...
    onMoveShouldSetPanResponder: (_, g) => Math.abs(g.dx) > Math.abs(g.dy) * 1.6 && Math.abs(g.dx) > 8,
    ...
    onPanResponderMove: (_, g) => {
      ...
      if (off >= MAX_FUTURE_OFFSET && g.dx < 0) {
        const r = Math.sign(g.dx) * Math.pow(Math.abs(g.dx), 0.6) * 1.2;
        cardX.setValue(r); dateSlide.setValue(r * 0.15); cardOpacity.setValue(1 - Math.min(Math.abs(r) / W, 0.15));
      } else { cardX.setValue(g.dx); dateSlide.setValue(g.dx * 0.2); cardOpacity.setValue(1 - Math.abs(g.dx) / W * 0.3); }
    },
    onPanResponderRelease: (_, g) => {
      const { dayOffset: off, goBack: gb, goForward: gf } = navRef.current;
      const fwd = off < MAX_FUTURE_OFFSET && (g.dx < -SWIPE_THRESHOLD || (g.vx < -VELOCITY_THRESHOLD && g.dx < -15));
      const bck = g.dx > SWIPE_THRESHOLD || (g.vx > VELOCITY_THRESHOLD && g.dx > 15);
      if (fwd) gf(); else if (bck) gb();
      else Animated.parallel([
        Animated.spring(cardX, { toValue: 0, tension: 300, friction: 28, useNativeDriver: true }),
        Animated.timing(cardOpacity, { toValue: 1, duration: 180, useNativeDriver: true }),
        Animated.spring(dateSlide, { toValue: 0, tension: 300, friction: 28, useNativeDriver: true }),
      ]).start();
    },
  ...
  const info = labelFor(dayOffset, language);
    ...
    <View style={s.root}>
      ...
      <View style={{ flex: 1 }}>
        {activeTab === 'saved' ? <SavedScreen />
         ...
         : activeTab === 'search' ? <SearchScreen allEvents={allCollectedEvents} />
         : activeTab === 'timeline' ? <TimelineScreen allEvents={allCollectedEvents} />
         : initialLoad ? (
          <View style={s.loadingWrap}>
            <View style={[s.loadingPulse, { borderColor: theme.gold + '25' }]}>
              <Text style={{ color: theme.gold, fontSize: 22, opacity: 0.4 }}>{'\u25CB'}</Text>
            </View>
          </View>
        ) : (
          <View style={s.contentWrap} {...(activeTab === 'today' ? pan.panHandlers : {})}>
            {activeTab === 'today' ? (
              isEmpty || !todayEvent ? (
                <EmptyDay isToday={info.isToday} theme={theme} t={t} onRetry={() => {
                  prefetchCache.current = {};
                  fetchData(dayOffset, true).then(({ data, empty }) => { setEvents(data); setIsEmpty(empty); });
                }} />
              ) : (
                <Animated.View style={{ flex: 1, transform: [{ translateX: cardX }], opacity: cardOpacity }}>
                  <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false} scrollEventThrottle={16}
                    refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={handleRefresh} tintColor={theme.gold} colors={[theme.gold]} progressBackgroundColor={theme.card} />}>
...
const makeStyles = (theme: any, isDark: boolean) => StyleSheet.create({
  ...
  dayNumber: { fontSize: 17, fontWeight: '800', letterSpacing: -0.5 },
  dateTexts: { gap: 2 },
  dateLabel: { fontSize: 16, fontWeight: '700', letterSpacing: 0.2 },
  dateSubLabel: { fontSize: 12, fontWeight: '500', opacity: 0.6 },
  separator: { height: StyleSheet.hairlineWidth },
  contentWrap: { flex: 1, paddingHorizontal: 16, paddingTop: 12 },
  loadingWrap: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  loadingPulse: { width: 56, height: 56, borderRadius: 28, borderWidth: 2, alignItems: 'center', justifyContent: 'center' },
