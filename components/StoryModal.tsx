...
import { Animated, Dimensions, Linking, Modal, Platform, ScrollView, Share, StatusBar, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export interface StoryModalProps { visible: boolean; event: any; onClose: () => void; theme: any; allEvents?: any[]; }

const ImageViewer = ({ images, initialIndex, onClose }: { images: string[]; initialIndex: number; onClose: () => void }) => {
  const insets = useSafeAreaInsets(); const [currentIndex, setCurrentIndex] = useState(initialIndex);
  const scrollRef = useRef<ScrollView>(null); const bgOpacity = useRef(new Animated.Value(0)).current;
  ...
  return (<Modal visible transparent animationType="none" statusBarTranslucent onRequestClose={handleClose}><View style={{ flex: 1 }}><Animated.View style={[StyleSheet.absoluteFill, { backgroundColor: '#000', opacity: bgOpacity }]} /><ScrollView ref={scrollRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false} onMomentumScrollEnd={e => setCurrentIndex(Math.round(e.nativeEvent.contentOffset.x / W))} style={{ flex: 1 }}>{images.map((img, i) => (<View key={i} style={{ width: W, height: H, alignItems: 'center', justifyContent: 'center' }}><Image source={{ uri: img }} style={{ width: W, height: H }} contentFit="contain" transition={300} /></View>))}</ScrollView><View style={[iv.bar, { paddingTop: insets.top + 8 }]}><TouchableOpacity onPress={handleClose} style={iv.btn} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}><X color="#fff" size={20} strokeWidth={2.5} /></TouchableOpacity>{images.length > 1 && <Text style={iv.count}>{currentIndex + 1} / {images.length}</Text>}<View style={{ width: 40 }} /></View>{images.length > 1 && (<View style={[iv.dots, { paddingBottom: insets.bottom + 20 }]}>{images.map((_, i) => (<View key={i} style={[iv.dot, { backgroundColor: i === currentIndex ? '#fff' : 'rgba(255,255,255,0.3)', width: i === currentIndex ? 16 : 5 }]} />))}</View>)}</View></Modal>);
};
...
const Paragraphs = ({ text, theme }: { text: string; theme: any }) => {
  ...
  return (<>{chunks.map((para, i) => i === 0 ? (<View key={i} style={st.paraWrap}><View style={{ flexDirection: 'row', alignItems: 'flex-start' }}><Text style={[st.dropCap, { color: theme.gold }]}>{para.charAt(0)}</Text><Text style={[st.paraText, { color: theme.text, flex: 1 }]}>{para.slice(1)}</Text></View></View>) : (<View key={i} style={st.paraWrap}><Text style={[st.paraText, { color: theme.text }]}>{para}</Text></View>))}</>);
};
...
export const StoryModal = ({ visible, event, onClose, theme, allEvents: allEventsProp }: StoryModalProps) => {
  const { language } = useLanguage(); const { isDark } = useTheme(); const insets = useSafeAreaInsets();
  const contextEvents = useAllEvents();
  const allEvents = (allEventsProp && allEventsProp.length > 0) ? allEventsProp : contextEvents;
  const [galleryIndex, setGalleryIndex] = useState(0); const [viewerVisible, setViewerVisible] = useState(false); const [viewerStartIndex, setViewerStartIndex] = useState(0);
  const [relatedEvent, setRelatedEvent] = useState<any>(null);
  const scrollY = useRef(new Animated.Value(0)).current;
  const { saveEvent, removeEvent, isSaved } = useSavedStore(); const markEventRead = useGamificationStore(s => s.markEventRead);
  const eventId = event ? getEventId(event) : null;
  useEffect(() => { if (visible && eventId && event) { markEventRead(eventId, event.category ?? 'history', String(event.eventDate ?? event.event_date ?? event.year ?? '').trim()); } }, [visible, eventId]);
  useEffect(() => { if (visible) { setGalleryIndex(0); setRelatedEvent(null); } }, [visible, eventId]);
  if (!event) return null;
  const saved = isSaved(eventId!); const toggleSave = () => saved ? removeEvent(eventId!) : saveEvent(event);
  const gallery: string[] = event.gallery ?? []; const year = String(event.eventDate ?? event.event_date ?? event.year ?? '').trim();
      ...
      <View style={[st.root, { backgroundColor: theme.background }]}>
        <Animated.View style={[st.floatingBar, { paddingTop: insets.top + 6, backgroundColor: headerBg }]} pointerEvents="box-none">
          ...
          <View style={st.actions}>
            ...
            <View style={{ flex: 1 }} />
            <TouchableOpacity onPress={() => Share.share({ message: `${title} (${year})\n\n${narrative.substring(0, 140)}…\n\nDaily History App` }).catch(() => {})} style={st.btn}><Share2 color="#fff" size={17} strokeWidth={2} /></TouchableOpacity>
            <TouchableOpacity onPress={toggleSave} style={[st.btn, { marginLeft: 10 }, saved && st.btnActive]}><Bookmark color={saved ? '#ffd700' : '#fff'} fill={saved ? '#ffd700' : 'transparent'} size={17} strokeWidth={2} /></TouchableOpacity>
          </View>
        </Animated.View>
        <Animated.ScrollView bounces showsVerticalScrollIndicator={false} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: false })} scrollEventThrottle={16} contentContainerStyle={{ paddingBottom: insets.bottom + 40 }}>
          <View style={st.heroWrap}>
            <Animated.View style={{ height: HERO_H + 40, marginTop: -40, transform: [{ translateY: heroParallax }] }}>
              {gallery.length > 0 ? (<ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} onMomentumScrollEnd={e => setGalleryIndex(Math.round(e.nativeEvent.contentOffset.x / W))}>{gallery.map((img, i) => (<TouchableOpacity key={i} activeOpacity={0.9} onPress={() => { setViewerStartIndex(i); setViewerVisible(true); }} style={{ width: W, height: HERO_H + 40 }}><Image source={{ uri: img }} style={{ width: W, height: HERO_H + 40 }} contentFit="cover" transition={400} /></TouchableOpacity>))}</ScrollView>) : <View style={{ width: W, height: HERO_H + 40, backgroundColor: '#0d0d0f' }} />}
            </Animated.View>
            <LinearGradient colors={['rgba(0,0,0,0.15)', 'transparent', 'rgba(0,0,0,0.55)', 'rgba(0,0,0,0.96)']} locations={[0, 0.3, 0.65, 1]} style={StyleSheet.absoluteFill} pointerEvents="none" />
          ...
          <View style={[st.body, { backgroundColor: theme.background }]}>
            <View style={st.chips}>{year !== '' && <StatChip label="YEAR" value={year} theme={theme} />}<StatChip label="CATEGORY" value={category} theme={theme} /></View>
            <View style={st.ruleRow}><View style={[st.rule, { backgroundColor: theme.border }]} /><View style={[st.diamond, { backgroundColor: theme.gold }]} /><View style={[st.rule, { backgroundColor: theme.border }]} /></View>
            <Paragraphs text={narrative || 'No story available.'} theme={theme} />
            {sourceUrl && (<TouchableOpacity onPress={() => Linking.openURL(sourceUrl).catch(() => {})} activeOpacity={0.75} style={[st.wikiBtn, { borderColor: theme.gold + '40', backgroundColor: theme.gold + '0D' }]}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}><BookOpen size={16} color={theme.gold} strokeWidth={2} /><View><Text style={[st.wikiLabel, { color: theme.subtext }]}>WANT TO READ MORE?</Text><Text style={[st.wikiTitle, { color: theme.gold }]}>Continue on Wikipedia</Text></View></View><Text style={{ fontSize: 18, color: theme.gold }}>→</Text></TouchableOpacity>)}
            ...
            <TouchableOpacity onPress={toggleSave} activeOpacity={0.8} style={[st.saveBtn, { backgroundColor: saved ? '#ffd700' + '18' : theme.card, borderColor: saved ? '#ffd700' + '60' : theme.border }]}><Bookmark size={15} color={saved ? '#ffd700' : theme.subtext} fill={saved ? '#ffd700' : 'transparent'} strokeWidth={2} /><Text style={[st.saveBtnText, { color: saved ? '#ffd700' : theme.subtext }]}>{saved ? 'Saved to library' : 'Save to library'}</Text></TouchableOpacity>
...
const st = StyleSheet.create({
  root: { flex: 1 }, floatingBar: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 100, paddingHorizontal: 16, paddingBottom: 10 },
  floatingTitle: { fontSize: 13, fontWeight: '600', letterSpacing: 0.2, textAlign: 'center', marginBottom: 8 }, actions: { flexDirection: 'row', alignItems: 'center' },
  ...
  heroWrap: { height: HERO_H, overflow: 'hidden' }, heroOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 22 },
  ...
  dotsRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 4 }, dot: { height: 5, borderRadius: 3 },
  ...
  chipLabel: { fontSize: 7, fontWeight: '700', letterSpacing: 2 }, chipValue: { fontSize: 14, fontWeight: '800', letterSpacing: 0.5 },
  ruleRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 26, gap: 10 }, rule: { flex: 1, height: StyleSheet.hairlineWidth },
  diamond: { width: 5, height: 5, transform: [{ rotate: '45deg' }], opacity: 0.7 },
  paraWrap: { marginBottom: 20 }, dropCap: { fontSize: 42, lineHeight: 42, fontFamily: Platform.OS === 'ios' ? 'Georgia' : 'serif', fontWeight: '800', marginRight: 4, marginTop: 2, opacity: 0.85 },
  ...
  wikiLabel: { fontSize: 8, fontWeight: '700', letterSpacing: 2, marginBottom: 3 }, wikiTitle: { fontSize: 14, fontWeight: '700', letterSpacing: 0.2 },
