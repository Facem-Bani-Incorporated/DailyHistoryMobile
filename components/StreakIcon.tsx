...
const L: Record<string, Record<string, string>> = {
  en: {
    ...
    watchAd: 'Watch ad for +50 XP', adLoading: 'Loading...',
  },
  ro: {
    ...
    watchAd: 'Vizionează reclamă +50 XP', adLoading: 'Se încarcă...',
  },
  fr: {
    ...
    watchAd: 'Regarder une pub +50 XP', adLoading: 'Chargement...',
  },
  de: {
    ...
    watchAd: 'Werbung ansehen +50 XP', adLoading: 'Laden...',
  },
  es: {
    ...
    watchAd: 'Ver anuncio +50 XP', adLoading: 'Cargando...',
  },
};
  ...
  return (
    <View style={{ width: RS, height: RS, alignItems: 'center', justifyContent: 'center' }}>
      <Svg width={RS} height={RS}>
        ...
        <Circle cx={RS / 2} cy={RS / 2} r={RR} stroke="url(#rg)" strokeWidth={RW} fill="none" strokeDasharray={RC} strokeDashoffset={RC * (1 - p)} strokeLinecap="round" rotation="-90" origin={`${RS / 2}, ${RS / 2}`} />
      </Svg>
      <View style={{ position: 'absolute', alignItems: 'center' }}>
        <Text style={{ fontSize: 15, fontWeight: '900', color: ok ? '#34D399' : theme.text, letterSpacing: -0.5 }}>{ok ? '✓' : `${read}`}</Text>
        <Text style={{ fontSize: 7, fontWeight: '700', color: theme.subtext, opacity: 0.35, letterSpacing: 0.5, marginTop: 1 }}>/{total}</Text>
      </View>
    </View>
  );
};
...
const StreakModal = ({ visible, onClose }: { visible: boolean; onClose: () => void }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      Animated.stagger(100, [anim(fadeHero, slideHero, 100), anim(fadeStats, slideStats, 0), anim(fadeGoal, slideGoal, 0), anim(fadeList, slideList, 0)]).start();
    }
  }, [visible]);
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent={false} statusBarTranslucent>
      <View style={[sms.root, { backgroundColor: bg, paddingTop: ins.top }]}>
        <View style={sms.hdr}>
          <View style={{ width: 36 }} />
          <Text style={[sms.hdrTitle, { color: theme.text }]}>{tx(language, 'title')}</Text>
          <TouchableOpacity onPress={onClose} hitSlop={{ top: 16, bottom: 16, left: 16, right: 16 }} style={[sms.closeBtn, { backgroundColor: isDark ? '#181412' : '#F5F0E8', borderColor: brd }]}>
            <X size={15} color={theme.subtext} strokeWidth={2.5} />
          </TouchableOpacity>
        </View>

        <ScrollView contentContainerStyle={[sms.scroll, { paddingBottom: ins.bottom + 40 }]} showsVerticalScrollIndicator={false}>
          {/* Hero */}
          <Animated.View style={[sms.heroCard, { backgroundColor: '#0A0806', borderColor: brd, opacity: fadeHero, transform: [{ translateY: slideHero }] }]}>
            <View style={sms.lottieWrap}>
              {streak > 0 ? <LottieView source={{ uri: LOTTIE_URI }} autoPlay loop speed={0.8} style={sms.lottieBig} /> : <View style={sms.deadEmber}><View style={[sms.emberDot, { backgroundColor: isDark ? '#2A1F14' : '#3E2A18' }]} /></View>}
            </View>
            <View style={sms.heroTextWrap}>
              <Text style={sms.heroNumber}>{streak}</Text>
              <Text style={[sms.heroLabel, { color: accent }]}>{streak === 1 ? tx(language, 'day') : tx(language, 'days')}</Text>
            </View>
            <View style={[sms.heroDivider, { backgroundColor: brd }]} />
            <View style={sms.heroStatsRow}>
              <View style={sms.heroStat}><Ionicons name="trophy" size={13} color="#FFB300" /><Text style={[sms.heroStatVal, { color: '#fff' }]}>{longest}</Text><Text style={sms.heroStatLbl}>{tx(language, 'best')}</Text></View>
              <View style={[sms.heroStatDivider, { backgroundColor: 'rgba(255,255,255,0.08)' }]} />
              <View style={sms.heroStat}><Ionicons name="book" size={13} color="#7C6BC4" /><Text style={[sms.heroStatVal, { color: '#fff' }]}>{totalEventsRead}</Text><Text style={sms.heroStatLbl}>{tx(language, 'total')}</Text></View>
            </View>
          </Animated.View>

          {/* Goal */}
          <Animated.View style={[sms.goalCard, { backgroundColor: card, borderColor: brd, opacity: fadeGoal, transform: [{ translateY: slideGoal }] }]}>
            <View style={sms.goalHeader}>
              <Text style={[sms.goalTitle, { color: theme.text }]}>{tx(language, 'goal')}</Text>
              <View style={[sms.goalBadge, { backgroundColor: read >= GOAL ? '#34D39915' : (accent + '15') }]}>
                <Text style={[sms.goalBadgeT, { color: read >= GOAL ? '#34D399' : accent }]}>{read >= GOAL ? tx(language, 'done') : `${GOAL - read} ${tx(language, 'left')}`}</Text>
              </View>
            </View>
            <View style={sms.goalBody}>
              <ProgressRing read={read} total={GOAL} theme={theme} isDark={isDark} />
              <View style={{ flex: 1, gap: 8 }}>
                <Text style={[sms.goalMsg, { color: sub }]}>{read >= GOAL ? tx(language, 'doneMsg') : tx(language, 'goMsg')}</Text>
                <View style={sms.barRow}>{Array.from({ length: GOAL }).map((_, i) => <View key={i} style={[sms.barSeg, { backgroundColor: i < read ? (read >= GOAL ? '#34D399' : accent) : (isDark ? '#1A1510' : '#F0EBE3') }]} />)}</View>
              </View>
            </View>
          </Animated.View>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={[sms.rewardText, { color: isRewardedReady ? accent : theme.subtext }]}>
                {isRewardedReady ? tx(language, 'watchAd') : tx(language, 'adLoading')}
              </Text>
            </View>
            <View style={[sms.rewardBadge, { backgroundColor: accent + '20' }]}>
              <Text style={[sms.rewardBadgeT, { color: accent }]}>+50 XP</Text>
            </View>
          </TouchableOpacity>

          {/* Recently read */}
          {readEventsToday.length > 0 && (
            <Animated.View style={[sms.listCard, { backgroundColor: card, borderColor: brd, opacity: fadeList, transform: [{ translateY: slideList }] }]}>
              <View style={sms.listHeader}><Text style={[sms.listTitle, { color: theme.text }]}>{tx(language, 'recent')}</Text><View style={[sms.listCount, { backgroundColor: dimBg }]}><Text style={[sms.listCountT, { color: accent }]}>{readEventsToday.length}</Text></View></View>
              {items.map((eid, i) => { const { name, date } = cleanName(eid); return (<View key={eid}>{i > 0 && <View style={[sms.listDiv, { backgroundColor: brd }]} />}<View style={sms.listItem}><View style={[sms.listCheck, { backgroundColor: accent + '12' }]}><Ionicons name="checkmark" size={12} color={accent} /></View><View style={{ flex: 1 }}><Text style={[sms.listName, { color: theme.text }]} numberOfLines={2}>{name}</Text>{date !== '' && <Text style={[sms.listDate, { color: sub }]}>{date}</Text>}</View></View></View>); })}
              {readEventsToday.length > 4 && (<><View style={[sms.listDiv, { backgroundColor: brd }]} /><TouchableOpacity onPress={() => setShowAll(v => !v)} activeOpacity={0.6} style={sms.moreBtn}><Text style={[sms.moreTxt, { color: accent }]}>{showAll ? tx(language, 'less') : `${tx(language, 'more')} (${readEventsToday.length - 4})`}</Text><Ionicons name={showAll ? 'chevron-up' : 'chevron-down'} size={13} color={accent} /></TouchableOpacity></>)}
            </Animated.View>
          )}

          <View style={[sms.tipCard, { borderColor: brd }]}><Ionicons name="sparkles" size={13} color={accent} style={{ marginTop: 1 }} /><Text style={[sms.tipText, { color: sub }]}>{tx(language, 'tip')}</Text></View>
        </ScrollView>
      </View>
    </Modal>
  );
...
const sms = StyleSheet.create({
  ...
  // Rewarded ad button
  rewardBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16, borderRadius: 20, borderWidth: 1, marginBottom: 12 },
  rewardText: { fontSize: 14, fontWeight: '700', letterSpacing: 0.2 },
  rewardBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10 },
  rewardBadgeT: { fontSize: 12, fontWeight: '900', letterSpacing: 0.3 },
    ...
    <>
      <TouchableOpacity onPress={onPress} activeOpacity={0.8}
        ...
        <Animated.View style={{ transform: [{ scale: tap }] }}>
          {active ? <LottieView source={{ uri: LOTTIE_URI }} autoPlay loop speed={0.7} style={{ width: 26, height: 26 }} /> : <View style={{ width: 18, height: 18, alignItems: 'center', justifyContent: 'flex-end' }}><View style={{ width: 7, height: 9, borderRadius: 3.5, backgroundColor: isDark ? '#3A3025' : '#C8BFB0', opacity: 0.4 }} /></View>}
        </Animated.View>
        {streak > 0 && <View style={[ic.badge, { backgroundColor: active ? '#E8850A' : (isDark ? '#3A3530' : '#C0B8A8') }]}><Text style={ic.badgeT}>{streak}</Text></View>}
      </TouchableOpacity>
...
const ic = StyleSheet.create({
  ...
  badge: { position: 'absolute', top: -5, right: -5, minWidth: 18, height: 18, borderRadius: 9, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.15, shadowRadius: 3, elevation: 3 },
